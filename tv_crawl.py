...
host="localhost",
database="compare_products",
user="saikat123",
password="1234")

s = sh.Shortener()

page = requests.get("https://www.flipkart.com/televisions/pr?sid=ckf%2Cczl&marketplace=FLIPKART&otracker=product_breadCrumbs_Televisions&page=1")
p=1
while(True):
   if p > 30:
      break
      ...
      for i in range(3):
         try:
            cursor.execute("""Insert Into mobiles_mobiles
                           ...
                           """+Star.get_text()+""", 
                           '"""+Rating.get_text()+"""',
                           '"""+In_The_Box+"""') ON CONFLICT (company_name) DO NOTHING""")
